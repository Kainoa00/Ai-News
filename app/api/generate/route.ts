 ...
 * GET /api/generate
 *
 * Called automatically by Vercel Cron every Wednesday at 14:00 UTC (7 AM MST).
 * Vercel Cron always sends GET requests with Authorization: Bearer <CRON_SECRET>.
 */
export async function GET(req: NextRequest) {
  ...
  console.log("[generate] Cron triggered — starting weekly edition via Grok...");
  const startTime = Date.now();
  ...
  const duration = ((Date.now() - startTime) / 1000).toFixed(1);
  console.log(`[generate] Done. ${inserted.length}/${successful.length} inserted in ${duration}s.`);
